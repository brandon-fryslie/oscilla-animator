...
The codebase has three separate types (Connection, Publisher, Listener), separate registries for lenses and adapters, default sources as metadata rather than blocks, and a stub V2 adapter. The authoritative spec in `compiler-final/ARCHITECTURE-RECOMMENDATIONS.md` provides clear refactor paths, but none have been started yet.
...
// Plus publishers and listeners in BusStore
```
...
| `src/editor/stores/PatchStore.ts` | Replace connections/publishers/listeners with edges | 48-49, all connection methods |
