...
|------|----------|--------|--------|
| Scan inputs | Find unconnected inputs | ✅ isInputDriven() checks wires and listeners | ✅ |
...
- Should be simplified to just check for wires/listeners and error if missing
...
1. **Pass 6 (pass6-block-lowering.ts:312-318)** - Remove defaultSource fallback
   ...
   - Simplify to: check for wire/listener, error if missing
