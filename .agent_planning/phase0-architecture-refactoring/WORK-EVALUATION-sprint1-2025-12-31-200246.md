...
- **PatchStore.ts:132-154**: Computed getters (wireEdges, publisherEdges, listenerEdges)
...
- ✅ Migration helpers: connectionToEdge(), publisherToEdge(), listenerToEdge() (edgeMigration.ts:51-139)
- ✅ Reverse helpers: edgeToConnection(), edgeToPublisher(), edgeToListener() (edgeMigration.ts:152-235)
...
**Criteria Met**:
- ✅ `PatchStore.edges: Edge[]` replaces connections/publishers/listeners (line 67)
- ✅ Unified `addEdge()` action (lines 1132-1171)
- ✅ Unified `removeEdge()` action (lines 1180-1213)
- ✅ Computed getters: `wireEdges`, `publisherEdges`, `listenerEdges` (lines 132-154)
...
- Code still uses separate wire/listener lookups (lines 548-661)
...
2. **PatchStore edge management tests** (`stores/__tests__/PatchStore.edges.test.ts`)
   ...
   - Test edge filtering (wireEdges, publisherEdges, listenerEdges)
...
- Pass 8 still uses legacy `wires.find()` and `listeners.find()` ❌
...
**Current**: Separate wire and listener lookups
```typescript
...
```typescript
// Check for any edge to this input (wire or bus listener)
