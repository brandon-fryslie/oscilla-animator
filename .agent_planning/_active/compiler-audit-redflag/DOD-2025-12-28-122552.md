...
- [ ] `CycleTimeRoot` block compiler calls `builder.setTimeModel()` during lowering
...
- [ ] Test: Compile cyclic patch (CycleTimeRoot with periodMs=1000)
  - `compiledProgram.ir.timeModel.kind === 'cyclic'`
  - `compiledProgram.ir.timeModel.periodMs === 1000`
  - Compilation succeeds without errors
...
- [ ] Manual test: Cyclic patch runtime behavior
  - Create patch: CycleTimeRoot(1000ms) + phase-driven animation
  - Player shows phase looping 0→1 every second
  - Wrap events emitted correctly
  - Scrubbing shows repeating cycle
