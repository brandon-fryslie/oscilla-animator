...
2. **BusStore Full Migration** (P1) - deleteBus, updateBus, updatePublisher, updateListener, lens operations
...
- [ ] BlockAdded event still fires for UI listeners
...
- [ ] Undo recreates block with all connections, publishers, listeners, default sources
- [ ] Lane membership restored on undo
- [ ] GraphCommitted event fires with correct diff (blocks, connections, bindings)
- [ ] BlockRemoved event still fires for UI listeners
- [ ] All existing tests pass after migration

### Technical Notes
- `removeBlockCascade()` already handles all dependencies (connections, publishers, listeners, default sources, lanes)
...
deleteBus(busId: string): void {
  ...
  this.publishers = this.publishers.filter(p => p.busId !== busId);  // ❌ Direct
  this.listeners = this.listeners.filter(l => l.busId !== busId);  // ❌ Direct
}
```
...
3. Verify cascade removes publishers and listeners in correct order

### Acceptance Criteria
- [ ] `deleteBus()` uses `tx.removeBusCascade()` exclusively
- [ ] No direct array filtering on buses/publishers/listeners
- [ ] User can delete bus → undo → redo successfully
- [ ] Undo recreates bus with all publishers and listeners
- [ ] Publisher sortKeys preserved on undo
- [ ] GraphCommitted event fires with correct diff
- [ ] BusDeleted event still fires for UI listeners
- [ ] All existing tests pass after migration

### Technical Notes
- `removeBusCascade()` already implemented in TxBuilder (lines 398-421)
- Cascade generates Many op with correct order (publishers, listeners, bus)
- Inverse recreates in reverse order (bus, listeners, publishers)
...
## P1-3: Migrate updatePublisher() and updateListener()
...
### Description
Migrate publisher/listener updates to use transaction-based Update ops. These are currently the only bus routing operations that bypass runTx().

### Current Implementation Analysis
```typescript
// src/editor/stores/BusStore.ts (lines 267-282)
updatePublisher(publisherId: string, updates: Partial<Publisher>): void {
  const index = this.publishers.findIndex(p => p.id === publisherId);
  const existing = this.publishers[index];
  const updated = { ...existing, ...updates };
  this.publishers[index] = updated;  // ❌ Direct array mutation
}

// Similar pattern for updateListener()
```

**Problem**: Array index replacement doesn't go through transaction system.

### Implementation Steps
1. Migrate updatePublisher:
   ```typescript
   updatePublisher(publisherId: string, updates: Partial<Pick<Publisher, 'enabled' | 'sortKey' | 'adapterChain' | 'lensStack'>>): void {
     runTx(this.root, { label: 'Update Publisher' }, tx => {
       const pub = this.root.busStore.publishers.find(p => p.id === publisherId);
...
2. Migrate updateListener (same pattern)

3. Verify sortKey changes (publisher reordering) work correctly

### Acceptance Criteria
- [ ] `updatePublisher()` uses `tx.replace()` for all updates
- [ ] `updateListener()` uses `tx.replace()` for all updates
- [ ] No direct array index assignments
- [ ] User can enable/disable publisher → undo → redo successfully
- [ ] User can reorder publishers (sortKey) → undo → redo successfully
...
### Technical Notes
- Publisher/listener updates are moderate frequency (not as high as param drags)
...
**Complexity**: Large
**Dependencies**: P1-3
**Reason**: Lens stack operations modify nested arrays within listener objects, requiring careful transaction design
...
describe('Undo/Redo Integration - Buses', () => {
  it('undoes deleteBus with cascade', () => {
    ...
    const pubId = rootStore.busStore.addPublisher(busId, ...);
    const listenerId = rootStore.busStore.addListener(busId, ...);
    ...
    expect(rootStore.busStore.publishers.find(p => p.id === pubId)).toBeDefined();
    expect(rootStore.busStore.listeners.find(l => l.id === listenerId)).toBeDefined();
  });
  ...
  it('undoes updatePublisher', () => {
    const busId = rootStore.busStore.createBus('TestBus');
    const pubId = rootStore.busStore.addPublisher(busId, ...);

    // Update publisher
    rootStore.busStore.updatePublisher(pubId, { enabled: false });
    expect(rootStore.busStore.publishers.find(p => p.id === pubId)!.enabled).toBe(false);

    // Undo
    rootStore.historyStore.undo();
    expect(rootStore.busStore.publishers.find(p => p.id === pubId)!.enabled).toBe(true);
  });
});
```
...
#### Bus Operations
5. **Create Bus → Add Publisher → Undo**
   - Create bus
   - Add publisher from block
   - Press Cmd+Z (undo add publisher)
   - Verify publisher removed
   - Press Cmd+Z (undo create bus)
   - Verify bus removed

6. **Delete Bus → Undo**
   - Create bus with publishers/listeners
   - Delete bus
   - Press Cmd+Z
...
- [ ] User can undo: createBus, deleteBus, updateBus, add/remove/update publisher/listener
...
2. **Event Ordering Changes**
   ...
   - *Validation*: Test event listeners
...
1. **Day 1-2**: P1-1 (deleteBus) + P1-2 (updateBus) + P1-3 (updatePublisher/Listener)
...
| P1-3: updatePublisher/Listener | Medium |
