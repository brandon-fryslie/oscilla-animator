...
1. Add to TxBuilder class:
   - `removeBlockCascade(blockId)`:
     ...
     - Remove all bus publishers/listeners for block
     - Remove default sources for block inputs
     - Remove block from lanes
     - Remove block itself

   - `removeBusCascade(busId)`:
     - Remove all publishers for bus
     - Remove all listeners for bus
     - Remove bus itself

2. Query helpers for dependency discovery:
   - `getConnectionsForBlock(blockId)`: find all wires
   - `getPublishersForBlock(blockId)`: find all publishers
   - `getListenersForBlock(blockId)`: find all listeners
...
- [ ] Publisher/listener add/remove generates correct ops
- [ ] Lens stack modifications generate Update ops for listener entity
- [ ] deleteBus uses removeBusCascade helper
- [ ] All existing bus tests pass
- [ ] Bus-related events still fire correctly

#### Technical Notes
- Lens stack changes are Update ops (modify listener entity)
- Publisher reordering is Update op (sortKey change)
- Bus deletion cascade removes all publishers/listeners first
...
#### Implementation
1. Add keyboard event listeners in Editor component
...
- May need global keyboard listener (not input-focused)
...
### Low Risks
1. **Event System Compatibility**: Existing event listeners should continue working
