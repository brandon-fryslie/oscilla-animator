...
**Workflow Recommendation**: 🔴 **PAUSE** - Type system issues must be resolved before continuing migration
...
|-------|--------|----------------|
| `just test` | ❌ FAIL | 4 TypeScript compilation errors block test execution |
| Manual IR lowering verification | ⚠️ BLOCKED | Cannot run until compilation passes |
...
'expression': 'expression',  // ❌ Not in TypeDomain
'Expression': 'expression',  // ❌ Not in TypeDomain
'waveform': 'waveform',      // ❌ Not in TypeDomain
'Waveform': 'waveform',      // ❌ Not in TypeDomain
...
### ✅ Step 1: Audit Current Lowering Usage - COMPLETE
...
- `AddSignal.ts` - ✅ `inputsById?.a ?? inputs[0]`
- `MulSignal.ts` - ✅ `inputsById?.b ?? inputs[1]`
- `SubSignal.ts` - ✅ `inputsById?.a ?? inputs[0]`
- `Oscillator.ts` - ✅ `inputsById?.phase ?? inputs[0]`, etc.
...
### ❌ Step 2: Add Typed PortCatalog Helper - NOT STARTED
...
### ⚠️ Step 3: Strict Port Contract by Default - PARTIALLY IMPLEMENTED
...
### ❌ Step 4: Add `inputsById` and `outputsById` Support - 50% COMPLETE

**Status**: HALF IMPLEMENTED

**✅ What exists** (`inputsById` support):
...
**❌ What's missing** (`outputsById` support):
...
export interface LowerResult {
  readonly outputs: readonly ValueRefPacked[];  // ❌ Required, not optional
  readonly declares?: BlockDeclarations;
  // ❌ Missing: outputsById field
}
...
### ⚠️ Step 5: Migrate Reference Blocks - 50% COMPLETE

**Migrated** (4 blocks):

| Block | `inputsById` | `outputsById` | Port Catalog | Status |
|-------|--------------|---------------|--------------|--------|
| `Oscillator.ts` | ✅ Fallback pattern | ❌ Positional only | ⚠️ Separate file | PARTIAL |
| `AddSignal.ts` | ✅ Fallback pattern | ❌ Positional only | ❌ Inline defs | PARTIAL |
| `MulSignal.ts` | ✅ Fallback pattern | ❌ Positional only | ❌ Inline defs | PARTIAL |
| `SubSignal.ts` | ✅ Fallback pattern | ❌ Positional only | ❌ Inline defs | PARTIAL |
...
### ❌ Step 6: Update Tests - NOT STARTED
...
### ❌ Step 7: Document the Pattern - NOT STARTED
...
C. Map them to existing domains (`'expression'` → `'string'`?, `'waveform'` → `'string'`?)
...
### 🟡 High Priority (Before Migrating More Blocks)
...
### 🔵 Low Priority (After Full Migration)
...
- ✅ `signal-math.test.ts` - Should pass (uses fallback pattern)
- ⚠️ Missing tests - Port validation, `outputsById`, contract enforcement

## Files Modified in Recent Commits

Recent work (git log):
- `820ca71` - Add default sources to IR block defs
- `f715d82` - Remove legacy time references from misc other docs
...
**What works**:
- ✅ `inputsById` infrastructure functional
- ✅ 4 blocks successfully use fallback pattern
- ✅ Port catalog structure established (Oscillator)
- ✅ Strict contract enforcement works (when enabled)

**What's broken**:
- ❌ TypeScript compilation fails (4 errors)
- ❌ `outputsById` not implemented
- ❌ Strict-by-default inverted from plan
- ❌ No `definePortCatalog` helper (drift risk)
