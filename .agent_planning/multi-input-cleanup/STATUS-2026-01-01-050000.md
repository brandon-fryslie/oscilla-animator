...
export interface Bus {
  combineMode: BusCombineMode;  // No 'when' needed - always combines publishers
}
```

**Why Different**:
1. **Buses ALWAYS combine** publishers (when N > 1) - no "when: 'multi' vs 'always'" choice
2. **Buses CANNOT use 'error' mode** - multiple publishers are expected
...
const bus: Bus = {
  combine: { when: 'multi', mode: 'error' }  // ❌ Buses can't forbid publishers
};
...
- **Bus.combineMode**: Always combines publishers (no 'when' policy needed)
...
**Why Different**:
- Buses always combine publishers → no 'when' choice
