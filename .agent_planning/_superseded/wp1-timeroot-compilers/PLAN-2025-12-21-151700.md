...
### ✅ Complete
- TimeRoot block definitions (FiniteTimeRoot, CycleTimeRoot, InfiniteTimeRoot)
...
- Auto-insert default CycleTimeRoot on patch creation
- Comprehensive compiler tests (259 lines)
- Feature flag `requireTimeRoot: true` ENABLED

### ❌ Missing (Per Spec & User Requirements)
- **CycleTimeRoot missing outputs**: `cycleT`, `wrap`, `cycleIndex` (spec lines 105-110)
...
|-----------|-----------------|---------------|-----|
| CycleTimeRoot outputs | 5 outputs (t, cycleT, phase, wrap, cycleIndex) | 2 outputs (systemTime, phase) | Missing: cycleT, wrap, cycleIndex |
...
- [ ] CycleTimeRoot returns complete TimeOutputs bundle
...
**Energy Derivation**:
- **CycleTimeRoot**: `energy = 1.0` (constant baseline for steady loops)
...
## [P0] Add Missing CycleTimeRoot Outputs
...
Extend CycleTimeRoot block definition and compiler to emit all spec-required outputs. Currently only emits `systemTime` and `phase`. Must add `cycleT`, `wrap`, `cycleIndex`, and integrate with TimeOutputs bundle.
...
- [ ] CycleTimeRoot block definition adds 4 new outputs (cycleT, wrap, cycleIndex, energy)
...
**Why**: Currently must poll `progress >= 1.0` in downstream blocks. Event-driven is cleaner and aligns with CycleTimeRoot wrap event pattern.
...
**Publications**:
- CycleTimeRoot: `phaseA` → bus phaseA, `wrap` → bus pulse, `energy` → bus energy
...
- [ ] Integration test: CycleTimeRoot auto-publishes phase to phaseA bus
- [ ] Integration test: Wrap events from CycleTimeRoot appear on pulse bus
...
**Status**: Not Started
**Effort**: Small (1 day)
**Dependencies**: [P0] CycleTimeRoot outputs, [P0] FiniteTimeRoot end event
...
- [ ] CycleTimeRoot outputs: systemTime, cycleT, phase, wrap, cycleIndex, energy (6 total)
...
- [ ] Manual wiring: Can connect CycleTimeRoot.wrap to non-bus consumers (e.g., debug logger)

### Technical Notes

**File**: `src/editor/blocks/time-root.ts`

**CycleTimeRoot additions** (after line 102):
```typescript
outputs: [
...
- [ ] Test: CycleTimeRoot wrap event fires at period boundary (loop mode)
- [ ] Test: CycleTimeRoot wrap event fires at pingpong direction flips
- [ ] Test: CycleTimeRoot cycleIndex increments correctly over multiple cycles
...
describe('CycleTimeRoot - wrap events', () => {
  it('fires wrap event at period boundary (loop mode)', () => {
    const result = CycleTimeRootBlock.compile({ params: { periodMs: 1000, mode: 'loop' } });
...
describe('CycleTimeRoot - auto-publications', () => {
  it('returns autoPublications for phaseA, pulse, energy', () => {
    const result = CycleTimeRootBlock.compile({ params: { periodMs: 3000, mode: 'loop' } });
...
- [ ] Load patch with CycleTimeRoot → TimeConsole shows phase ring UI with loop indicator
...
- [ ] Period change in CycleTimeRoot updates UI phase ring radius
- [ ] Screenshots: Capture all three UI modes for documentation

### Technical Notes

**Test Patches**:
1. Empty patch → Auto-inserts CycleTimeRoot (8s) → Verify phase ring
2. Delete CycleTimeRoot, add FiniteTimeRoot (5s) → Verify progress bar
...
└────────────┬────────────────────────┘
             ...
             v                                      v
┌─────────────────────────────┐   ┌──────────────────────────────┐
│ [P0] TimeOutputs Bundle     │   │ [P0] CycleTimeRoot Outputs   │
└────────────┬────────────────┘   └──────────┬───────────────────┘
             │                               │
             │                               │
             v                               v
...
- [P0] CycleTimeRoot missing outputs (wrap, cycleIndex, cycleT)
...
**Deliverable**: CycleTimeRoot auto-publishes to phaseA/pulse/energy, tests prove correctness.
...
2. ✅ CycleTimeRoot emits: systemTime, cycleT, phase, wrap, cycleIndex, energy
