...
- Defines three TimeRoot types: `FiniteTimeRoot`, `CycleTimeRoot`, `InfiniteTimeRoot`
...
- "A patch loops only if it has a CycleTimeRoot"
...
- Defines CycleTimeRoot with `periodMs`, `mode` inputs
...
- Uses names like: `CycleTimeRoot`, `GridDomain`, `StableIdHash`, `RenderInstances2D`
...
- CycleTimeRoot shows `phaseOffset` and `drift` as inputs (both are Signals)
...
**Q13:** What's the default period for CycleTimeRoot? (8s is used in Golden Patch)
...
|------------------|--------|-------|
| CycleTimeRoot compiler | NOT IMPLEMENTED | Block defined, no compiler |
