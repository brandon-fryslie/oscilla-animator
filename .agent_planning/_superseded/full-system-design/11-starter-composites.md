...
**Internal:**
- DomainN(n=rows*cols) → PositionMapGrid
...
**Internal:**
- DomainFromSVGSample(asset, sampleCount) → (optional) position field from sampling

**Outputs:** domain, pos, bounds

**Exposed:** asset, sampleCount

**Bus suggestions:** sampleCount usually static (avoid count jank)
...
**Internal:**
- FieldHash01ById → FieldMapNumber(fn="scale") to range [-amount, +amount]
...
**Internal:**
- hash → maprange [0, 2π] then scale by amount
...
**Internal:**
- treat phaseA as oscillator → map to [min,max]
...
**Internal:**
- phaseB → map to hue shift or color lerp endpoints

**Outputs:** Signal<color> (or Signal<number> hue bias if using HSL)
...
**Internal:**
- hash → hue offset; apply to base color
...
- expects domain, pos
- generates:
  - size = Per-Element Size Scatter × Breathing Scale (or energy lens)
...
**Internal:**
- same as Dots Renderer but shape="path" and uses pathAsset
...
**Internal:**
- EnvelopeAD(trig=pulse, attack, decay)
...
## Recommended "First Demo" Assembly
...
- Ship with 10 macros (1–5, 9–10, 12, 14–15, 17) and add the rest later
...
GridPoints is not a fundamental primitive; it's a common preset.
...
## Source Primitives Decomposition

What we're calling "Points" is really two different things:
1. A population exists (stable element identity, count, ordering)
2. Those elements have a position mapping (Field)

The clean separation:
- **DomainFactory** (creates the population)
- **Field Mappers** (compute per-element attributes like position, size, rotation)
- **Renderers** (decide what is drawn per element)

This is the axis of variation:
