...
- [ ] New patch creation auto-inserts CycleTimeRoot with `periodMs: 8000`
...
- [ ] CycleTimeRoot derives phase via modulo (no wrapping in player)
...
- [ ] Exports: `getSortedPublishers`, `combineSignalArtifacts`, `combineFieldArtifacts`, `validateCombineMode`

**Publisher Sorting Function**:
- [ ] `getSortedPublishers(busId, allPublishers, includeDisabled)` implemented
...
- [ ] Test: 3 publishers with same sortKey → sorted by id alphabetically
...
- [ ] `BusStore.getPublishersByBus(busId)` calls `getSortedPublishers()` from busSemantics
- [ ] BusStore removes local sorting logic (delegates to module)
- [ ] BusStore results identical before/after refactor
- [ ] Test: BusStore.getPublishersByBus() returns publishers in sortKey+id order

**Compiler Integration**:
- [ ] `compileBusAware.ts` imports `getSortedPublishers`, `combineSignalArtifacts`, `combineFieldArtifacts`
- [ ] Local `sortPublishers()` function deleted (lines 83-91)
...
- [ ] Test: BusStore and compiler return identical ordering for same publisher set
- [ ] Test: Tie-breaker verified (publishers `{id:'b',sortKey:0}`, `{id:'a',sortKey:0}` → sorted `['a','b']`)
...
- [ ] Tests for `getSortedPublishers`: basic sort, tie-breaker, filtering
...
- [ ] Bus visualization shows correct publisher ordering
