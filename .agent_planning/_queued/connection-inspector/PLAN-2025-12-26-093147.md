...
- `Connection` (wire) - STATUS-2025-12-26-103000.md §3
...
**Dependencies**: None
**Spec Reference**: N/A (new feature) • **Status Reference**: STATUS-2025-12-26-103000.md §2 (Inspector System Architecture)
...
**Spec Reference**: N/A (new feature) • **Status Reference**: STATUS-2025-12-26-103000.md §1 (What's Missing)
...
```tsx
ConnectionInspector
├── InspectorContainer (header with "Connection" title, back button, type color)
├── Connection Endpoints Section
│   ├── Source display (block name, port name, type badge)
│   └── Target display (block/bus name, port/bus name, type badge)
├── Actions Section
│   ├── Disconnect button
│   └── Swap button (context-dependent behavior)
└── Lens Editor Section (bus connections only)
    └── Embedded LensChainEditor (collapsible "Add Lens" panel)
```

**Connection Type Handling**:
- **Wire**: Show source block/port → target block/port, no lens section
...
- [ ] E2E test: Select connection → inspect → disconnect → verify removed
...
**Spec Reference**: N/A (new feature) • **Status Reference**: STATUS-2025-12-26-103000.md §2 (Lens Editing Components)
...
- [ ] E2E test: Edit lens in embedded mode → verify onChange called → verify no "Done" button

##### Technical Notes

The current LensChainEditor already applies changes immediately (STATUS-2025-12-26-103000.md §2 lines 58-70). The "Done" button only closes the popover. In embedded mode, there's no popover to close, so the button is unnecessary.
...
**Spec Reference**: N/A (new feature) • **Status Reference**: STATUS-2025-12-26-103000.md §5 (Entry Points in Port/Block Inspector)
...
- [ ] E2E test: Select block → click port → click connection in list → verify ConnectionInspector shows
...
**Effort**: Medium (3-5 days)
**Dependencies**: P0-1, P0-2 (Connection selection state, ConnectionInspector component)
**Spec Reference**: N/A (new feature) • **Status Reference**: STATUS-2025-12-26-103000.md §4 (ModulationTable Cell Click Handling)
...
**Spec Reference**: N/A (new feature) • **Status Reference**: STATUS-2025-12-26-103000.md §1, Ambiguities §5
...
**Navigation Contexts**:
1. **From Port Inspector**: Back → restore block selection + port selection
2. **From ModulationTable**: Back → restore cell focus (no inspector change)
3. **From Bus Inspector**: Back → restore bus selection
4. **Direct selection**: Back → clear selection (no prior context)
...
**Spec Reference**: N/A (new feature) • **Status Reference**: STATUS-2025-12-26-103000.md §1
...
**Visual Design**:
- **Wire** (block→block): Blue header, wire icon (⚡)
...
- [ ] E2E test: Open each connection type → verify correct color/icon
...
**Effort**: Medium (3-5 days)
**Dependencies**: P0-2 (ConnectionInspector component)
**Spec Reference**: N/A (new feature) • **Status Reference**: STATUS-2025-12-26-103000.md §1, Ambiguities §2
...
- [ ] E2E test: Swap wire connection → select new target → verify connection updated
...
├── P0-2 (ConnectionInspector)
│   ├── P1-1 (Port Inspector Entry)
│   ├── P1-2 (ModulationTable Entry)
│   ├── P1-3 (Back Navigation)
│   ├── P2-1 (Color Coding)
│   └── P2-2 (Swap UX)
└── P0-3 (Embedded Lens Editor)
    └── (used by P0-2)
```

**Critical Path**: P0-1 → P0-2 → P1-1/P1-2 → P1-3
...
1. User can click connection in port inspector → see ConnectionInspector
2. User can click bound cell in ModulationTable → see ConnectionInspector
