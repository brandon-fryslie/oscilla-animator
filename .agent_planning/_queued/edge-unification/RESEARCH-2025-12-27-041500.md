...
| **Publisher** | Block output port | Bus | BusStore.publishers | Broadcast to bus |
| **Listener** | Bus | Block input port | BusStore.listeners | Receive from bus |
...
type GraphEdge =
  ...
  | { kind: 'publish'; publisher: Publisher }
  | { kind: 'subscribe'; listener: Listener };
```
...
- **Publisher**: 1:N (one output → N listeners through bus)
- **Listener**: N:1 (N publishers combined → one input)
...
export class EdgeQueryService {
  ...
  // Get all edges for a block (wires, publishers, listeners)
  ...
  // Get value source for an input (wire, listener, or default)
...
- Bus type editing
- Store mutations (create wire vs create publisher)
