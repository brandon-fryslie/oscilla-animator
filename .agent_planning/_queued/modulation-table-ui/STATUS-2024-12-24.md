...
- Line 191: INPUT rows use `createGroupKey(blockDef.subcategory ?? 'Other', block.id)` → prefixes like `'Other:'`, `'Render:'`, `'TimeRoot:'`
...
Current: Separate `◂`/`▸` buttons for collapsing sections.
...
In `types.ts` line 424: `'palette': '■'` (black square)
User wants: Rainbow emoji (🌈)
