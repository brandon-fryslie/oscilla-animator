...
- Publisher/Listener use `PortRef` for `from`/`to` fields
...
- TimeRoot blocks exist (CycleTimeRoot, FiniteTimeRoot, InfiniteTimeRoot)
...
- CycleTimeRoot auto-publish: `phase -> phaseA`, `wrap -> pulse`
- Current TimeRoot doesn't auto-create these publishers
...
- Port type lookup
- Bus publisher/listener indexing
...
- `src/editor/kernel/ops.ts` defines all 26 Op types:
  ...
  - Binding: PublisherAdd/Remove/Update, ListenerAdd/Remove/Update
...
**Implementation Evidence:**
- `AdapterStep[]` on publishers/listeners
- `LensInstance[]`/`lensStack` on listeners
- Types separated in `types.ts`

**Gaps:**
- [ ] Lenses not fully type-preserving enforced
- [ ] Publisher lenses not implemented (only listener)
...
- Implement CycleTimeRoot -> phaseA/pulse auto-publishing
