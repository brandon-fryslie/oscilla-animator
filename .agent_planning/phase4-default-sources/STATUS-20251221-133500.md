...
**Status**: ✅ Type definitions are complete and in production code.
...
compile(args: {
  id: BlockId;
  params: Record<string, unknown>;  // ⬅️ REMOVE IN PHASE 4
...
**Current logic**: Wire → Bus → Error (if required)
**Target logic**: Wire → Bus → **Default Source** → Error
...
- `deriveDefaultSource()` - Convert ParamSchema → DefaultSource
...
|-------|--------|----------|-------|
| Phase 1: Type Definitions | ✅ COMPLETE | No | Slot.defaultSource exists in types.ts |
| Phase 2: Block Definitions | 🟡 40% DONE | **Yes** | Need remaining 30 blocks migrated |
| Phase 3: Inspector UI | ❓ UNKNOWN | **Maybe** | UI may still write to block.params |
...
**Effort**: 1-2 days (36 compilers × 15-30 min each)
...
|---------|-------|--------|
| `src/editor/compiler/blocks/domain/*.ts` | 23 | Replace `params.X` → `inputs.X?.value` |
| `src/editor/compiler/blocks/signal/*.ts` | 8 | Replace `params.X` → `inputs.X?.value` |
| `src/editor/compiler/blocks/rhythm/*.ts` | 2 | Replace `params.X` → `inputs.X?.value` |
...
1. ✅ **Verify Phase 2 completion** - Check all blocks have defaultSource
2. ✅ **Verify Phase 3 status** - Ensure Inspector doesn't write to params
3. ⚠️ **Create migration script** - Automate params → inputs refactor
4. ⚠️ **Baseline visual tests** - Capture screenshots before migration

### During Migration
1. 🔄 **One compiler at a time** - Don't batch, test incrementally
2. 🔄 **Dual-mode validation** - Keep params as fallback initially
3. 🔄 **Continuous testing** - Run tests after each compiler migration

### Post-Migration
1. 📝 **Monitor production** - Watch for parameter-related errors
2. 📝 **Collect feedback** - Ask users about missing parameters
3. 📝 **Plan Phase 5** - Cleanup paramSchema after validation period

---

## 11. Next Steps

**Blocking on Phase 2**: ⚠️ Cannot start Phase 4 until block definitions complete
...
- CycleTimeRoot, FiniteTimeRoot (time)
