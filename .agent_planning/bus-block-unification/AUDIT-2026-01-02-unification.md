...
|---------|-----|---------------------|
| Multiple writers | Publishers | Multiple incoming wires |
...
Publisher interface          →  Edge (port→BusBlock.in)
Listener interface           →  Edge (BusBlock.out→port)
Endpoint.kind === 'bus'      →  Eliminated (all PortRef)
```

**Files with dual systems**:
- `src/editor/types.ts:165-270` - Bus, Publisher, Listener, Endpoint all still defined
...
BusStore (facade)
  ...
  ├── publishers: Publisher[] (local, NOT delegated)
  └── listeners: Listener[] (local, NOT delegated)
```

**Problem**: Facade only covers `buses`, not publishers/listeners. This creates:
...
**Finding P1**: BusStore facade is incomplete - publishers/listeners still local.
...
export function pass7BusLowering(
  ...
  publishersOrEdges?: readonly Publisher[] | readonly Edge[], // Union type!
...
| File | Lines | Impact |
|------|-------|--------|
| `pass1-normalize.ts` | 63, 72 | Publisher sorting |
| `pass6-block-lowering.ts` | 417 | Writer kind check |
| `pass7-bus-lowering.ts` | 167, 274 | Legacy edge detection |
| `resolveWriters.ts` | 162-169 | Bus listener classification |
| `PatchStore.ts` | 155, 164 | publisherEdges/listenerEdges getters |
...
5. [ ] Remove Bus, Publisher, Listener interfaces
6. [ ] Remove Patch.buses, Patch.publishers, Patch.listeners arrays
...
| Legacy Publisher/Listener in BusStore | MEDIUM | Should have been migrated with facade |
...
| Bus/Publisher/Listener types | ~150 | 0 | 150 lines |
...
- Same evaluation order (publishers before listeners)
...
|------|----------|--------|
| `getPublishersFromLegacyEdges()` | pass7-bus-lowering.ts | DEPRECATED - delete after Sprint 3 |
| `BusStore.publishers` array | BusStore.ts | Should delegate to edges |
| `BusStore.listeners` array | BusStore.ts | Should delegate to edges |
| `PatchStore.publisherEdges` getter | PatchStore.ts | Refactor to filter BusBlock edges |
| `PatchStore.listenerEdges` getter | PatchStore.ts | Refactor to filter BusBlock edges |
...
├── Remove Bus/Publisher/Listener types
...
- BusStore facade incomplete (publishers/listeners not delegated)
