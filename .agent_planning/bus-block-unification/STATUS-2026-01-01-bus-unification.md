...
export interface Bus {
  ...
  combine: CombinePolicy;      // How to merge N publishers
  defaultValue: unknown;        // Fallback when N=0
  sortKey: number;              // Publisher ordering
...
- `publishers: Publisher[]` - Deprecated (use edges)
- `listeners: Listener[]` - Deprecated (use edges)
...
| 'port' | 'bus' | Publisher (deprecated type still exists) | Legacy + new |
| 'bus' | 'port' | Listener (deprecated type still exists) | Legacy + new |
...
- Computed: `wireEdges`, `publisherEdges`, `listenerEdges` (filters on Endpoint.kind)
...
export type Writer =
  ...
  | { kind: 'bus'; listenerId; busId }       // ← Would become 'wire' from BusBlock
...
- Used by Pass 7 (bus lowering) for N > 1 publishers
...
2. For each input: enumerate writers (wires, bus listeners, defaults)
3. If N > 1: create combine node via `createCombineNode()`
4. Wire combine output to block input

**After unification:**
- Bus listeners would be wires from BusBlock.out
- Logic identical, just simpler writer enumeration

#### Pass 7: Bus Lowering (`src/editor/compiler/passes/pass7-bus-lowering.ts`)

**Current:**
1. For each Bus: enumerate publishers via `getPublishersFromEdges(busId, edges)`
2. Sort publishers by sortKey/weight
3. If N > 1: create combine node via `createCombineNode()`
...
```
Publisher₁ ──┐
Publisher₂ ──┼─> [Bus "energy"] ─┬─> Listener₁
Publisher₃ ──┘   (combine: sum)  └─> Listener₂
```
...
#### 3. Deprecate Publisher/Listener Types

**Already deprecated** (comments say "Use Edge instead"), but can now **delete entirely**:
- `Publisher` interface (src/editor/types.ts:343)
- `Listener` interface (src/editor/types.ts:375)
- `BusStore.publishers` array
- `BusStore.listeners` array
- `PatchStore.publisherEdges` computed
- `PatchStore.listenerEdges` computed
...
**Current:** Manages Bus entities and deprecated publisher/listener arrays
...
export class BusStore {
  ...
  // DELETE: publishers, listeners (redundant with edges)
  // DELETE: addPublisher, removePublisher, etc.
...
**Current:**
```typescript
// Enumerate writers (wires, bus listeners, defaults)
...
**Savings:** ~50 lines of bus listener handling deleted.
...
- After: Same, but simpler (no bus listener enumeration)
...
| `Publisher` interface | src/editor/types.ts:343 | 25 | Redundant with Edge |
| `Listener` interface | src/editor/types.ts:375 | 20 | Redundant with Edge |
| Pass 7 bus lowering | src/editor/compiler/passes/pass7-bus-lowering.ts | 400+ | BusBlock handled by Pass 6 |
| Bus listener handling in Pass 6 | pass6-block-lowering.ts | ~50 | All inputs are wires |
| `Writer.kind === 'bus'` case | resolveWriters.ts | 20 | Only 'wire' and 'default' |
| `BusStore.publishers` | stores/BusStore.ts | 30 | Redundant |
| `BusStore.listeners` | stores/BusStore.ts | 30 | Redundant |
| `PatchStore.publisherEdges` | stores/PatchStore.ts | 5 | Redundant |
| `PatchStore.listenerEdges` | stores/PatchStore.ts | 5 | Redundant |
...
2. **Edge unification** (ROADMAP: unify-connections-edge)
   ...
   - Need to fully deprecate Connection/Publisher/Listener
...
**Testing:** Ensure bus evaluation order preserved (publishers before listeners).
...
- [ ] BusBlock.in accepts multiple publishers with correct combine
- [ ] BusBlock.out connects to listeners correctly
...
- [ ] **BusBlock accepts multiple publishers** - input.combine policy works
- [ ] **BusBlock output connects to listeners** - normal port→port edges
