...
**Goal**: Eliminate the final direct cross-store mutation in PatchStore by introducing the `BlockReplaced` event and migrating selection update logic to an event listener.
...
- No event listener for block replacement selection updates
...
#### P0-3: Add BlockReplaced Event Listener

**Status**: Not Started
**Effort**: Small (15 minutes)
**Dependencies**: P0-1 (event definition)
**Spec Reference**: Event listener pattern (RootStore lines 77-82) • **Status Reference**: STATUS-2025-12-20-161838.md lines 104-117

##### Description
Add `BlockReplaced` event listener in `RootStore.setupEventListeners()` to update `uiStore.selectedBlockId` when a selected block is replaced. This mirrors the existing `BlockRemoved` listener pattern.

##### Acceptance Criteria (REQUIRED)
- [ ] Event listener added in `RootStore.setupEventListeners()` (after line 82)
- [ ] Listener checks if `event.oldBlockId === this.uiStore.uiState.selectedBlockId`
- [ ] If true, listener updates selection to `event.newBlockId`
- [ ] If false, listener does nothing (preserves existing selection)
- [ ] Listener is synchronous (no async operations)

##### Technical Notes
- Add immediately after existing `BlockRemoved` listener (line 77-82)
...
**Dependencies**: P0-2 (event emission), P0-3 (event listener)
**Spec Reference**: Decoupling requirement (STATUS lines 38-48) • **Status Reference**: STATUS-2025-12-20-161838.md lines 38-48

##### Description
Remove the direct UIStore mutation from `PatchStore.replaceBlock()` (lines 516-517). Selection update will now occur via the `BlockReplaced` event listener in RootStore.
...
**Status**: Not Started
**Effort**: Small (30 minutes)
**Dependencies**: P0-3, P0-4 (event listener and mutation removal)
...
##### Description
Add integration tests in `src/editor/stores/__tests__/RootStore.events.test.ts` to verify selection updates via `BlockReplaced` event listener maintain correct UX behavior.
...
##### Technical Notes
- Create RootStore instance with event listeners set up
...
- Test uses same pattern as existing `BlockRemoved` listener tests (lines 77-82)
...
3. P0-3: Add event listener in RootStore (15 min)
4. P0-4: Remove direct UIStore mutation (5 min)
5. **Manual verification**: Replace a block in UI, verify selection follows

**Acceptance**: Event emitted, listener working, no direct coupling

---

### Sprint 2: Test Coverage (1 hour)
**Goal**: Comprehensive test coverage for new event and listener
...
- **If tests fail after P0-4**: Temporarily revert lines 516-517, debug listener logic, then re-remove
- **If event payload wrong**: Compare with existing `BlockRemoved` event pattern, verify field names
- **If listener doesn't fire**: Verify event added to `EditorEvent` union type
...
- [ ] `RootStore.setupEventListeners()` has `BlockReplaced` listener
...
- [ ] Inline comments in event listener (if needed)
...
- Listeners registered in `RootStore.setupEventListeners()` only
- Listeners must be synchronous (no async handlers)
- Listeners must not mutate domain store state (only UI/coordination state)
- Listeners must be idempotent (safe to call multiple times)

### Test Coverage Standards
- Every event must have unit test for emission
- Every event must have unit test for payload correctness
- Every event listener must have integration test
...
- Event listener count: 3 → 4 (BlockReplaced listener added)
...
| P0-3: Add Listener | 15 min | Very High |
