...
- **Functionality**:
  ...
  - Creates bus publishers/listeners (lines 278-315)
...
### MISSING: LogStore Event Listener
**Status**: NOT_STARTED
...
2. Set up listener externally (e.g., in editor initialization)
...
3. **`src/editor/__tests__/macro-log-clear.test.ts`** - MacroExpanded listener test
...
**Concern**: Listener error could break emit flow
...
3. **Event listener tests** - Verify listeners called with correct payloads
...
### Priority 4: Wire LogStore Listener (REQUIRED)
...
- **BindingAdded/Removed** - Bus listeners
...
5. Wire up MacroExpanded → LogStore.clear() listener
6. Add test coverage
