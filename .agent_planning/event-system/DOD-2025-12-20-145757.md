...
#### Event Emission
- [ ] `BusStore.addPublisher()` emits `BindingAdded` with `direction: 'publish'` AFTER publisher created
- [ ] `BusStore.addListener()` emits `BindingAdded` with `direction: 'subscribe'` AFTER listener created
- [ ] `BusStore.removePublisher()` emits `BindingRemoved` with `direction: 'publish'` AFTER removal
- [ ] `BusStore.removeListener()` emits `BindingRemoved` with `direction: 'subscribe'` AFTER removal
- [ ] Event payloads include all required fields (bindingId, busId, blockId, port, direction)

#### Testing
- [ ] Unit test: `addPublisher()` emits `BindingAdded` with correct payload
- [ ] Unit test: `addListener()` emits `BindingAdded` with correct payload
- [ ] Unit test: `removePublisher()` emits `BindingRemoved` with correct payload
- [ ] Unit test: `removeListener()` emits `BindingRemoved` with correct payload
...
- [ ] `RootStore.setupEventListeners()` subscribes to `BusDeleted` event
- [ ] Event listener checks if deleted bus was selected and clears `uiStore.selectedBusId`
...
- [ ] New tests added to appropriate test files:
  ...
  - `src/editor/stores/__tests__/RootStore.test.ts` (event listeners)
...
- [ ] Add bus publisher → `BindingAdded` event with direction='publish'
- [ ] Remove bus listener → `BindingRemoved` event with direction='subscribe'
