...
Remove the single-input invariant from PatchStore.connect() and BusStore.addListener() by removing the `disconnectInputPort` call. This enables multiple edges to the same input port permanently (no feature flag).
...
- [ ] BusStore.addListener() no longer calls disconnectInputPort
