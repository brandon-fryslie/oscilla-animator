...
**Effort Estimate**: 2-3 days (14-22 hours)
...
- Same semantics as bus publishers (busSemantics.ts)
...
**Dependencies**: None
**Spec Reference**: ARCHITECTURE-PROPOSAL.md lines 80-85 • **STATUS Reference**: STATUS-2025-12-31-193238.md § "Minimal Implementation Path" lines 241-256

#### Description

Add environment variable `VITE_ENABLE_MULTI_INPUT` to control multi-input block behavior. When `false` (default), preserve existing single-input invariant. When `true`, allow multiple edges to same input port.

This feature flag enables safe incremental rollout and allows reverting to old behavior if issues arise.

**Current Evidence** (STATUS lines 78-103):
- Single-input invariant enforced at PatchStore.ts:905-907
- Same invariant enforced at BusStore.ts:346-348
- `disconnectInputPort` implementation at PatchStore.ts:853-865
...
connect(from: Endpoint, to: Endpoint) {
  if (!ENABLE_MULTI_INPUT) {
    // Old behavior: enforce single-input invariant
...
**Files to modify**:
- `src/editor/stores/PatchStore.ts` (connect method)
- `src/editor/stores/BusStore.ts` (addListener method)
- `.env.example` (document flag)

**Migration safety**:
- Single-input is a subset of multi-input (zero breaking changes)
- Existing patches work unchanged
- Flag can be flipped at runtime (browser reload)
...
**Dependencies**: P1: Feature Flag for Multi-Input Support
...
Modify PatchStore.connect() and BusStore.addListener() to conditionally skip the `disconnectInputPort` call when `VITE_ENABLE_MULTI_INPUT=true`. This removes the architectural constraint that an input can have only one source.

**Current Evidence** (STATUS lines 78-103):
- PatchStore.ts:905-907: Wire connections enforce 1:1
- BusStore.ts:346-348: Bus listeners enforce 1:1
- PatchStore.ts:853-865: disconnectInputPort removes ALL wires + bus listeners

**Impact**:
- When flag ON: Multiple edges allowed to same input
- When flag OFF: Single-input invariant preserved (backward compat)
...
- Uses getSortedPublishers() for deterministic ordering
...
2. **Publisher sorting**: Deterministic (sortKey, weight) ordering
...
**Dependencies**: P1: Conditionally Remove Single-Input Invariant, P2: Extract Combine Logic from Pass 7
...
- Pass 6 resolves single input via: wire > listener > default source
...
**Critical path**: P0 → P1 → P1 → P2 → P2 (total ~18 hours)
