...
### Multi-Input Invariant Tests
- [ ] Multiple edges allowed to same input
- [ ] Existing patches continue to work (backward compatible)
...
2. **Multi-input support**: Multiple edges to same input allowed (permanent change)
3. **Combine logic**: Extracted from Pass 7, reusable for Pass 6
4. **Pass 6 updates**: Handles N=0, N=1, N>1 edge cases
5. **Validation**: combineMode validated against world/domain
...
- [ ] Load existing patch with single-input blocks
- [ ] Verify it compiles and runs correctly
- [ ] Verify N=1 case has no combine overhead
