...
|------|-------|-------------|
| `src/editor/__tests__/domain-pipeline.test.ts` | 23, 336-372 | Replace PhaseClockLegacy with CycleTimeRoot + PhaseClock |
...
#### Acceptance Criteria
- [ ] All 3 test files migrated to use CycleTimeRoot instead of PhaseClockLegacy
...
// NEW (correct):
const timeRoot = { type: 'CycleTimeRoot', params: { period: 2000 } };
...
**TimelineHint → TimeModel Mapping:**
```typescript
// OLD:
...
- [ ] Remove paramSchema from all 3 blocks (FiniteTimeRoot, CycleTimeRoot, InfiniteTimeRoot)
- [ ] Verify inputs have correct `defaultSource` values
...
1. **PhaseClockLegacy Test Migration (Category 1)** - Migrate 3 test files
   ...
   - Replace PhaseClockLegacy with CycleTimeRoot + PhaseClock pattern
