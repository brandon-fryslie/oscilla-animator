...
**Migration Path:** Replace with `CycleTimeRoot` + `PhaseClock` pattern
...
4. **PhaseClockLegacy** - Migrate tests first, then remove block
   - Update `domain-pipeline.test.ts` to use CycleTimeRoot
   - Update `composite-library.test.ts` to use CycleTimeRoot
   - Update `composite.expansion.test.ts` to use CycleTimeRoot
...
- B: Migrate to CycleTimeRoot (keep coverage, more work)
