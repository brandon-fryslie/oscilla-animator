...
**Current State:**
- Stub returns silent value (0) or first publisher value
- No transform chain application
- No real combine logic

**Required:**
- Read publisher source slots from ValueStore
- Apply transform chains to publisher values
...
**Challenge:**
- StepBusEval is lower-level (slots + publishers) than SignalExpr busCombine node
...
2. `src/editor/runtime/executor/steps/executeBusEval.ts` - Real combine logic
   - Read publisher values from ValueStore
...
5. `src/editor/runtime/executor/__tests__/executeBusEval.test.ts` (NEW)
   ...
   - Multiple publishers with ordering
   - (~20-25 tests)
