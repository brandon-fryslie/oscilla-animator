...
1. Ensure bus links are fully resolved:
   - Publisher → Bus → Consumer chains
   - Bus references in field expressions
   - Default bus values when no publishers
...
1. Add priority to bus publishers
2. Sort publishers by priority before combine
...
- `bus-priority.test.ts`: Publisher priority ordering
...
- [ ] Bus publishers sorted by priority
