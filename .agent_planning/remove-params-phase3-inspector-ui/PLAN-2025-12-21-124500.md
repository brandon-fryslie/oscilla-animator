...
function getInputConnectionState(
  ...
  const busListener = busStore.listeners.find(
    l => l.to.blockId === blockId && l.to.slotId === slotId
  );
  if (busListener) {
    return {
      kind: 'bus',
      listenerId: busListener.id,
      busId: busListener.busId,
      lens: busListener.adapterChain,
    };
  }
...
type InputConnectionState =
  ...
  | { kind: 'bus'; listenerId: string; busId: string; lens?: AdapterStep[] };
```
...
const InputsSection = observer(({ block }: { block: Block }) => {
  ...
  const handleDisconnect = useCallback((slotId: string, connectionState: InputConnectionState) => {
    if (connectionState.kind === 'wire') {
      ...
      store.busStore.removeListener(connectionState.listenerId);
    }
  }, [store]);
...
- **BusStore**: `listeners`, `buses` (for bus detection)
...
const connectionState = getInputConnectionState(...);  // Reads patchStore.connections, busStore.listeners
...
describe('getInputConnectionState', () => {
  ...
  it('returns bus when listener exists', () => { ... });
...
1. **BusBoard integration**: Show Default Sources as implicit publishers
...
| File | Why |
|------|-----|
| `src/editor/types.ts` | Connection, Publisher, Listener types |
| `src/editor/stores/PatchStore.ts` | connections, updateBlockParams() |
| `src/editor/stores/BusStore.ts` | listeners, buses, removeListener() |
