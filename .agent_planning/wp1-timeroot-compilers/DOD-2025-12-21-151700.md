...
- [ ] CycleTimeRoot compiler returns TimeOutputs-shaped object
...
### [P0] CycleTimeRoot Missing Outputs

- [ ] CycleTimeRoot block definition (time-root.ts) declares 6 outputs:
...
- [ ] CycleTimeRoot compiler returns `autoPublications` array with entries:
...
- [ ] InfiniteTimeRoot compiler returns autoPublications:
  - { busName: 'energy', artifactKey: 'energy', sortKey: 0 }
- [ ] Main compile.ts injects autoPublications into bus-aware compiler BEFORE manual publishers
- [ ] Auto-publications do NOT appear in patch.publishers (not persisted to patch data)
- [ ] Bus Board visualizes auto-publications with distinct styling (e.g., "system" badge or icon)
- [ ] Integration test: CycleTimeRoot phase appears on phaseA bus without manual publisher
- [ ] Integration test: Wrap events from CycleTimeRoot appear on pulse bus
- [ ] Integration test: Manual publisher to phaseA with sortKey=100 combines AFTER TimeRoot (sortKey=0)

### [P1] Update Block Definitions with All Outputs

- [ ] CycleTimeRoot (time-root.ts) outputs array includes all 6 outputs with correct types
...
- [ ] Manual wiring: Can connect CycleTimeRoot.wrap output to arbitrary consumer (not just bus)
- [ ] Manual wiring: Can connect FiniteTimeRoot.end output to arbitrary consumer

### [P1] Compiler Tests for New Outputs

- [ ] Test: CycleTimeRoot wrap event fires at period boundary in loop mode
...
- [ ] Test: CycleTimeRoot cycleIndex increments correctly over 3 cycles
  - t=0: cycleIndex=0
...
- [ ] Test: CycleTimeRoot autoPublications array contains phaseA, pulse, energy
...
- [ ] Period change in CycleTimeRoot updates phase ring radius in TimeConsole
...
- Minimum 3 integration tests:
  - CycleTimeRoot auto-publishes to phaseA bus
...
- Auto-publication injects 3 system publishers for CycleTimeRoot
...
4. Optional inputs (phaseOffset, drift) on CycleTimeRoot → Future WP
...
- How should manual publishers interact with auto-publications? (sortKey=100 default ensures TimeRoot is primary)
