...
1. Create a patch with bus publisher and listener
2. Compile to IR (not closure)
3. Verify StepBusEval appears in schedule
4. Run schedule and verify bus value changes when publisher changes
...
- [ ] Test changes publisher value and verifies bus value updates accordingly
...
describe("IR Bus Execution", () => {
  it("should emit StepBusEval and evaluate bus in IR mode", () => {
    // 1. Create patch with bus publisher + listener
    ...
    // 5. Change publisher and verify update
    runtime.valueStore.write(publisherSlot, 99);
    runtime.step(0.1);
...
- Verify reactive updates (bus value changes when publisher changes)
