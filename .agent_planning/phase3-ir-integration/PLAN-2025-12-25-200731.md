...
Implement Pass 7 to compile bus values into explicit IR combine nodes. This pass takes the flattened, sorted publishers from Pass 1 and creates `sigCombine` or `fieldCombine` nodes with the correct combine mode and deterministic publisher ordering.

**Key Insight:** Bus semantics are already implemented in `busSemantics.ts` (getSortedPublishers, combineSignalArtifacts, combineFieldArtifacts). This pass creates the IR representation of what those functions compute.

#### Acceptance Criteria (REQUIRED)

- [ ] Function `pass7BusLowering(unllinked: UnlinkedIRFragments, buses: Bus[], publishers: Publisher[]): IRWithBusRoots` exists in `src/editor/compiler/passes/pass7-bus-lowering.ts`
- [ ] For each bus, collect sorted publishers using `getSortedPublishers()` from `busSemantics.ts`
- [ ] Create `sigCombine` or `fieldCombine` node based on bus type (Signal vs Field)
- [ ] Apply publisher transform chains (adapters/lenses) as TransformChainIR references
...
- [ ] Test suite `pass7-bus-lowering.test.ts` with minimum 10 tests covering combine modes, publisher ordering, transform chains
...
**Publisher Transform Chain Handling:**

Publishers can have adapter chains and lens stacks. For Sprint 2, we'll create TransformChainIR references without full execution:

```typescript
function lowerPublisherToTerm(
  publisher: Publisher,
...
function createBusCombineNode(
  bus: Bus,
  publisherTerms: ValueRefPacked[],
  builder: IRBuilder
): ValueRefPacked {
  if (publisherTerms.length === 0) {
    // No publishers - use default value
  ...
  const sigIds = publisherTerms.filter(t => t.k === 'sig').map(t => t.id);
  const fieldIds = publisherTerms.filter(t => t.k === 'field').map(t => t.id);
...
- [ ] Function `pass8LinkResolution(irWithBusRoots: IRWithBusRoots, listeners: Listener[]): LinkedGraphIR` exists in `src/editor/compiler/passes/pass8-link-resolution.ts`
...
- [ ] Resolve all listener connections: input port → bus value ref
...
function resolveBlockInputs(
  ...
  for (const [blockIdx, block] of blocks.entries()) {
    for (const [portIdx, inputDef] of block.inputs.entries()) {
      ...
      // Check bus listener
      const listener = listeners.find(l => l.to.blockId === block.id && l.to.slotId === inputDef.name);
      if (listener) {
        const busIdx = /* ... */;
        const ref = busRoots.get(busIdx)!;
        // Apply listener transform chain if present
        refs.push(ref);
        continue;
      }
...
export function compileBusAwarePatch(
  ...
  if (errors.length === 0 && options?.emitIR === true) {
    try {
      ...
      const irWithBusRoots = pass7BusLowering(unllinked, buses, publishers);
      const linkedIR = pass8LinkResolution(irWithBusRoots, listeners);
...
- [ ] Test checks IR has correct number of blocks/buses/publishers/listeners
