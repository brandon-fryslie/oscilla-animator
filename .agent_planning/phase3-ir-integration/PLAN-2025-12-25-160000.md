...
**Types to add:**
- `NormalizedPatch` - Frozen block IDs, canonical publishers/listeners, default sources
...
**Acceptance Criteria (REQUIRED):**
- [ ] NormalizedPatch type is defined with blockIndexMap, blocks, wires, publishers, listeners, buses, defaultSources
...
- [ ] Default sources are attached to unwired inputs without bus listeners
- [ ] Publishers are canonicalized (enabled only, sorted by sortKey then id)
- [ ] Listeners are canonicalized (enabled only)
