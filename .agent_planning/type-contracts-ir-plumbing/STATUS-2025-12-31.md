...
if (refs[flatIdx] === undefined) {
  ...
  if (portDecl?.defaultSource === undefined) {
    errors.push({
      code: "MissingInput",
      message: `Input ${block.type}.${input.id} (block ${block.id}) has no connection, bus listener, or default source.`,
...
- Need to thread wire/listener data into Pass 6 context
