...
2. Bus listener resolution (lines 651-670) - adapter chain → lens stack
...
| Bus listener to unknown bus | ✅ YES | `DanglingBindingEndpoint` | Line 640-645 |
...
2. **Bus root validation missing:**
   - Plan says "Bus roots with missing publisher slots"
   - Current: Bus roots are created but not validated
   - **Evidence:** No validation in `pass7-bus-lowering.ts` or pass8
   - **Impact:** Buses without publishers could cause runtime errors
...
2. Validate bus roots have publishers (check busRoots against block outputs)
