...
2. If any block is FiniteTimeRoot/CycleTimeRoot/InfiniteTimeRoot, emit error:
...
### Publisher Ordering (COMPLETE)

**Location**: `src/editor/semantic/busSemantics.ts` (lines 41-60)

Canonical `getSortedPublishers()` function provides deterministic ordering:
...
private validateTimeRootDependencies(patch: PatchDocument): Diagnostic[] {
  ...
  // Check: Does TimeRoot have any incoming edges (wires or listeners)?
