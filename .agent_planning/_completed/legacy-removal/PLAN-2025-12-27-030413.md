...
5. Verify UI still works (handlers are local functions, not event listeners)
...
- [ ] Verify `BusBoard.tsx` and `PublishMenu.tsx` handlers are local (not event listeners) - no changes needed
...
- The `handleBusCreated` functions in UI files are **local callback functions**, not event listeners - they don't need changes
