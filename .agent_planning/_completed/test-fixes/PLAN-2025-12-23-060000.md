...
**Description**:
Authoring validator creates "Missing TimeRoot" diagnostic even when valid TimeRoot blocks exist in patch. Test creates mock PatchStore with CycleTimeRoot block but validator doesn't recognize it.

**Root Cause Evidence** (from STATUS):
- Test creates `{ id: 'block-1', type: 'CycleTimeRoot', label: 'TimeRoot', ... }`
...
**Acceptance Criteria**:
- [ ] Validator recognizes all three TimeRoot types: `CycleTimeRoot`, `FiniteTimeRoot`, `InfiniteTimeRoot`
