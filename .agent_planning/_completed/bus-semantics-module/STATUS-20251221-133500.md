...
The canonical `busSemantics` module has been successfully implemented and integrated. All duplicate logic has been eliminated from BusStore and compileBusAware. The module provides deterministic publisher ordering and combine semantics used consistently across UI and compiler.
...
✓ src/editor/semantic/__tests__/busSemantics.test.ts (41 tests) 17ms
  ✓ getSortedPublishers (9 tests)
    ✓ sorts by sortKey ascending
    ✓ uses id as tie-breaker when sortKey equal
    ✓ filters disabled publishers by default
    ✓ includes disabled publishers when requested
    ✓ filters by busId
    ✓ returns empty array when no publishers match
...
```
Uses canonical `getSortedPublishers()` from busSemantics module.
...
// Line 590: Publisher sorting in getBusValue()
...
2. **Performance benchmark** - Measure sorting performance with large publisher counts
...
### Publisher Ordering Flow

| Step | Component | Input | Processing | Output | Status |
|------|-----------|-------|------------|--------|--------|
| 1. Create Publisher | BusStore | User adds publisher | Assign sortKey (max+10) | Publisher object | ✅ |
| 2. Query Publishers | BusStore/Compiler | busId | `getSortedPublishers()` | Sorted array | ✅ |
| 3. Sort Logic | busSemantics | Publisher[] | Primary: sortKey, Secondary: id.localeCompare() | Deterministic order | ✅ |
| 4. UI Display | BusBoard | Sorted publishers | Render publisher list | Visual ordering | ✅ |
| 5. Compile | compileBusAware | Sorted publishers | Evaluate in order | Combined artifact | ✅ |
...
**Status**: COMPLETE ✅
**Evidence**:
- `src/editor/stores/BusStore.ts:503-505` - Calls `getSortedPublishers()`, no local sorting
...
- **Fix**: Both now use `getSortedPublishers()` with tie-breaker
...
- ✅ Uses real data structures (Publisher objects, Artifacts)
...
**Status**: COMPLETE ✅
**Evidence**:
- BusStore uses `getSortedPublishers()` for all queries
...
### Pagination & Lists
- ✅ **Tested**: `getSortedPublishers()` handles empty arrays, single items, many items
...
### Cleanup & Resources
- ✅ **No resources**: No connections, files, or listeners to clean up
...
- ✅ **Disabled publishers**: Explicit filtering with boolean flag
...
- [x] Single source of truth for publisher ordering
...
1. **Performance Optimization** (if needed)
   ...
   - Potential: Cache sorted publishers per bus, invalidate on change
...
- `src/editor/stores/BusStore.ts` - Uses `getSortedPublishers()`
...
**Consumers**:
- `BusStore.getPublishersByBus()` - UI layer queries
