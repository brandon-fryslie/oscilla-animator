...
- [ ] Read publisher source values from `ValueStore.read(pub.srcSlot)` for each publisher
- [ ] Filter enabled publishers (skip publishers where `enabled === false`)
- [ ] Implement all combine modes for signal buses:
  - [ ] `sum` - Sum all publisher values
  - [ ] `average` - Mean of all publisher values (sum / count)
...
- [ ] Handle silent value when no publishers enabled:
...
- [ ] 20-25 tests in `__tests__/executeBusEval.test.ts`:
  - [ ] `sum` mode with 2-3 publishers
  - [ ] `average` mode with 2-3 publishers
  - [ ] `min` mode with various values
  - [ ] `max` mode with various values
  - [ ] `last` mode (returns last publisher value)
  - [ ] Silent value handling (zero, default, const)
  - [ ] Transform chain application (scale/bias, ease, normalize)
  - [ ] Publisher ordering (sortKey determinism - ascending order)
  - [ ] Empty publisher list → silent value
  - [ ] All publishers disabled → silent value
  - [ ] Single publisher → combine mode applies (e.g. sum of 1 value = value)
