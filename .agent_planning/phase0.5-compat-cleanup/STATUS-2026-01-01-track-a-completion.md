...
2. `publisherToEdge()` - uses `convertLegacyTransforms()`
3. `listenerToEdge()` - uses `convertLegacyTransforms()`
...
- Line 354, 357: Publisher interface
- Line 386, 392: Listener interface
- Line 796, 799: Connection interface (deprecated type)

**Planned removal:**
- Remove from Edge interface
- Remove from PublisherData (Pass 7)
...
3. Update PublisherData in Pass 7
4. Update tests
...
- `src/editor/types.ts` - Remove from Publisher/Listener (lines 354, 357, 386, 392)
- `src/editor/types.ts` - Update Endpoint union (lines 232-233)

**Compiler:**
- `src/editor/compiler/passes/pass7-bus-lowering.ts` - Remove legacy fields from PublisherData
