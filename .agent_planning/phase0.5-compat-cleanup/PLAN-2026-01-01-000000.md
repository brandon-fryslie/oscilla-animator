...
- ⚠️ Three deprecated types still in use (Connection, Publisher, Listener)
...
Create a migration layer that converts legacy patches (with connections/publishers/listeners) to the new format (edges array) on load. This ensures all patches entering the system have edges populated.
...
function migratePatchToEdges(patch: Patch): Patch {
  ...
  const edges = [
    ...
    ...patch.publishers.map(publisherToEdge),
    ...patch.listeners.map(listenerToEdge)
  ];
...
Remove dual publisher retrieval logic and use edges directly. Delete the redundant `getPublishersFromEdges()` function.

##### Acceptance Criteria

- [ ] Lines 164-177 (dual publisher retrieval) removed
...
export interface NormalizedPatch {
  ...
  buses: Bus[];
  // connections, publishers, listeners REMOVED
}
```
...
Remove deprecated Connection, Publisher, Listener types and legacy arrays from Patch interface. This is the final breaking change for serialization format.
...
Delete the three deprecated interfaces (Connection, Publisher, Listener) from types.ts.
...
Remove connections/publishers/listeners arrays from Patch interface, making edges the only connection representation.
...
Remove connections/publishers/listeners observables from PatchStore.
...
Remove addConnection/removeConnection/updateConnection methods and their publisher/listener equivalents.
...
2. Create a publisher edge (port → bus)
3. Create a listener edge (bus → port)
4. Delete an edge
...
**Impact**: Undo/redo fails after removing Connection/Publisher/Listener
...
- `src/editor/types.ts` - Remove Connection/Publisher/Listener interfaces
...
- [ ] Architecture docs - Remove references to Connection/Publisher/Listener
