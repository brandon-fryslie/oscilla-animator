...
- [ ] Create publisher with adapter → compiles correctly
- [ ] Create publisher with lens → pre-combine transform works
- [ ] Publisher in IR mode → same behavior as closure mode

**Listener transforms**:
- [ ] Create listener with adapter → compiles correctly
- [ ] Create listener with lens → post-combine transform works
- [ ] Listener-only lens on wire → rejected with error
- [ ] Listener in IR mode → same behavior as closure mode
...
- Wire vs publisher vs listener: Same transform application code path
