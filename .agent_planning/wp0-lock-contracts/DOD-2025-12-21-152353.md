...
- [ ] Add test coverage: Define composite with CycleTimeRoot inside → expect error
...
**Sprint 2 delivers**: P1 Combine Mode Compatibility + P3 Multiple Publisher Validation
...
- [ ] Composite definition with CycleTimeRoot → clear error at definition time with `E_COMPOSITE_CONTAINS_TIMEROOT` diagnostic
...
- [ ] Patch with phaseA bus having 2 publishers → warning with `W_BUS_MULTIPLE_PUBLISHERS_CONTROL`
- [ ] Patch with energy bus having 5 publishers → no warning (expected behavior)
...
- `E_TIME_ROOT_BUS_LISTENER` - TimeRoot has bus listener (forbidden)
...
- `W_BUS_MULTIPLE_PUBLISHERS_CONTROL` - Control-plane bus has multiple publishers
