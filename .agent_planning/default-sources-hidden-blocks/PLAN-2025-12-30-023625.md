...
**Description**:
Create `injectDefaultSourceProviders()` function that converts DefaultSourceAttachments into normal blocks/wires/listeners. This is the core of the "hidden graph" strategy.
...
**Description**:
Implement logic to determine if an input is "undriven" (no wire AND no active listener). Only undriven inputs should get provider injection.
...
- [ ] Checks for listener: `patch.listeners.some(l => l.to.blockId === blockId && l.to.slotId === slotId && enabled)`
...
- An input with wire OR listener is "driven" = skip injection
...
**Description**:
Create helpers to generate deterministic IDs for injected wires, listeners, and blocks. No random UUIDs.
...
- [ ] Helper: `makeProviderListenerId(busId: string, providerId: string, inputId: string): string`
...
- Format matches existing wire/listener ID patterns
...
**In Scope**:
- Inject listeners for provider busInputs
...
#### P0: Inject bus listeners for provider inputs
...
**Description**:
For provider blocks that have busInputs (e.g., Oscillator needs 'phase' from 'phaseA' bus), inject listeners to wire bus to provider input.
...
- [ ] For each injected provider:
  ...
  - For each entry in spec.busInputs (inputSlotId → busName):
    ...
    - Create listener: `{ id, busId, to: { blockId: providerId, slotId: inputSlotId }, enabled: true }`
    - Add listener to `patch.listeners` array
- [ ] Listener ID: use `makeProviderListenerId()` helper
...
- Listener ID format: `lis:ds:${busId}->${providerId}:${inputId}`
...
**In Scope**:
- Detect when input is driven (wire or bus listener)
...
- [ ] Checks for listener: `store.listenerStore.listeners.some(...)`
...
- Check: wire exists OR enabled listener exists
...
- Tests existing wire/listener logic
...
- Verify bus listener injection works
...
- [ ] Verify bus listener injected in CompilerPatch

**Technical Notes**:
- This is comprehensive manual testing
- Tests: allowlist, UI, store, compiler injection, bus listeners, runtime
- Verify in DevTools: CompilerPatch has listener from phaseA to provider
...
- [ ] Open DevTools, check CompilerPatch:
  ...
  - Listener from phaseA bus to provider.phase
- [ ] No errors in console
...
- [ ] For each attachment:
  ...
  - Get target block publishers (buses target writes)
...
**Technical Notes**:
- File: `src/editor/defaultSources/validate.ts`
- Get target publishers: `rootStore.publisherStore.publishers.filter(p => p.blockId === targetBlockId)`
...
- [ ] **Scenario 4: No provider leakage**
  ...
  - Check Bus inspector: no extra listeners shown
...
- Sprint 11: Inject bus listeners
...
- Sprint 9-11 incremental approach: infrastructure → blocks → listeners
