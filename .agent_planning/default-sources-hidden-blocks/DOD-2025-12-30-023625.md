...
### Deliverable: Inject bus listeners for provider inputs

- [ ] For each injected provider: Get provider spec from allowlist, For each entry in spec.busInputs: Look up bus by name, Create listener, Add to `patch.listeners` array
...
- [ ] Open DevTools, check CompilerPatch: Oscillator provider exists, Wire to Circle.radius, Listener from phaseA to provider.phase
- [ ] No errors in console
...
- [ ] For each attachment: Get provider busInputs, Get target block publishers, If overlap exists: emit warning
...
- [ ] **Scenario 4: No provider leakage** - Check PatchBay (no providers), BlockLibrary (no DSConst*), Bus inspector (no extra listeners), diagnostics (friendly messages)
