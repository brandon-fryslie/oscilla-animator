...
- **Status**: COMPLETE for current model
- **What it does**:
  - Identifies unwired inputs (no wire, no bus listener)
...
- **What it does**:
  ...
  - Returns blocks/connections/buses/listeners/publishers
- **What's missing**: No injection of hidden provider blocks/wires/listeners
...
- ❌ MISSING: Logic to:
  ...
  4. Add bus listeners: busName → provider input
...
4. Legacy compiler sees normal wire/block/listener
5. IR compiler sees normal wire/block/listener
...
### 3. Bus Listener Injection - Always inject or only when needed?

**Question**: When a provider block has `busInputs`, do we always inject the listener or only when the bus exists?
...
- Option B: Skip listener, fall back to default value (lenient)
...
- Injecting blocks/wires/listeners into CompilerPatch
...
2. **Five critical ambiguities** need design decisions:
   ...
   - Bus listener injection (always inject or only when bus exists?)
