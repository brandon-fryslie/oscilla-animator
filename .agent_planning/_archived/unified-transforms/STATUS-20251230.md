...
2. **Lens scope mismatch**: Registry supports `publisher|listener` but wires already use lenses in practice
...
2. Bus listeners (line 662)
3. Bus publishers (line 1072)
...
1. **`src/editor/lenses/LensRegistry.ts`**
   - Change `LensScope = 'publisher' | 'listener'` to include `'wire' | 'lensParam'`
   - Update 27 lens definitions to declare scope compatibility explicitly:
     - Most lenses: `allowedScopes: ['wire', 'publisher', 'listener']`
     - Listener-only lenses (ease, mapRange, etc.): `allowedScopes: ['listener']`
...
- Option A: Wires support all publisher+listener lenses (most flexible)
- Option B: Wires are treated as listener-like (phase-window allowed)
- Option C: Wires have separate lens set (most restrictive)

**Recommendation**: Option A (wire = publisher ∪ listener allowed scopes)
...
2. **Limited Lens Support**:
   ...
   - Reject listener-only lenses in wire/publisher scopes

3. **IR Lowering Changes**:
   - Pass 6 (Block Lowering): Thread adapter chain through node creation
   - Pass 7 (Bus Lowering): Apply publisher lens stacks before combine
   - Pass 8 (Link Resolution): Apply wire/listener transform stacks
...
2. **Phase 4 (Centralize Application)**: Refactor working compiler code
   - **Mitigation**: Incremental migration (one scope at a time: wire → publisher → listener)
...
- Manual test: Apply listener-only lens to wire (rejected with clear error)
...
- [ ] Manual verification in `just dev`:
  ...
  - [ ] Publisher lens editing works and persists
  - [ ] Listener lens editing works and persists
...
- **Unified Bindings** (`plans/PLAN-UNIFIED-BINDINGS.md`): Completed, provides storage layer foundation
  - Transform storage already unified in `Connection`, `Publisher`, `Listener`, `LensParamBinding`
...
2. Create bus publisher with adapter → verify type conversion
3. Create bus listener with lens stack → verify chained transforms
...
5. Output: Bus artifact available to listeners
...
3. **Phase 3 (Wire Lens Scope)**: What should be allowed?
   - **Option A**: Wire = publisher ∪ listener scopes (most flexible)
   - **Option B**: Wire = listener-like (phase-window allowed)
