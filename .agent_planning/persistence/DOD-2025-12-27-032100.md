...
- [ ] Downloaded file contains all patch state: blocks, connections, lanes, buses, publishers, listeners, defaultSources, settings
...
- [ ] On confirm, patch is loaded via `RootStore.loadPatch()` and all state is restored:
  ...
  - [ ] Publishers and listeners are restored with correct bus references
...
- [ ] Save patch with buses/publishers/listeners → File contains routing data
...
- **Keyboard shortcuts**:
  - Add event listener to `window` or root component
