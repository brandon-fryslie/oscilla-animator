...
- Adapters run first, then lenses.
- Direct wire: `source -> adapters -> lenses -> target`
- Bus publisher: `publisher -> adapters -> lenses -> BusBlock.in` (pre-combine)
- Bus listener: `BusBlock.out -> adapters -> lenses -> listener` (post-combine)
