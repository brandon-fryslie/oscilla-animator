...
2. **Structural entities** - Created by the system to satisfy architectural invariants
...
- Role is structural, not presentational
...
- Don't reuse `kind` values across layers ambiguously (use `"structural"` at top level and `"defaultSource"` | `"wireState"` | ... inside)
...
**What is NOT rejected:** Structural blocks that:
...
Structural blocks are real blocks. They are created by the editor (not the compiler) to satisfy invariants. The UI may choose to filter them from certain views—that's a presentation choice, not an architectural one.
...
It does not see:
```
(user blocks, structural blocks, user edges, default edges)
```
...
- **auto**: Editor is allowed to delete/regenerate it as part of keeping structural intent satisfied
...
### Default Sources (§13)
Default Sources are structural blocks with `{ kind: "structural", meta: { kind: "defaultSource", target: { kind: "port", port } } }`.
They satisfy the invariant: **every input always has exactly one source**.

### Unified Transforms (§Unified-Transforms-Architecture)
Graph surgery creates infrastructure blocks. These are structural blocks.
...
A structural block compiles the same as a user block of the same type.
...
| **Structural block** | `role.kind === "structural"` - created by editor to satisfy invariants |
...
| **Auto edge** | `role.kind === "auto"` - editor-managed for structural integrity |

**Deprecated terms:**
- ~~Hidden block~~ → Use "structural block" (hidden implies invisible to compiler, which is wrong)
- ~~Phantom block~~ → Use "structural block"
- ~~Implicit block~~ → Use "structural block with specific meta.kind"
...
3. System-generated blocks get appropriate structural role
...
const visibleBlocks = patch.blocks.filter(b =>
  b.role.kind === "user" || settings.showStructuralBlocks
);
```
...
function validateRoleInvariants(patch: Patch): Diagnostic[] {
  ...
  // Default edges must reference structural defaultSource blocks
  for (const edge of patch.edges) {
    if (edge.role.kind === "default") {
      const sourceBlock = patch.blocks.find(b => b.id === edge.role.meta.defaultSourceBlockId);
      if (!sourceBlock || sourceBlock.role.kind !== "structural") {
        errors.push({ message: "Default edge must reference structural block" });
      }
    }
  }
  ...
  for (const block of patch.blocks) {
    if (block.role.kind === "structural" && block.role.meta.kind === "wireState") {
...
**Structural blocks are real blocks that exist in the patch data for architectural reasons.**
