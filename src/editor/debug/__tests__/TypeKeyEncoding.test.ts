...
describe('TypeKeyEncoding', () => {
  describe('TypeKeyTable', () => {
    it('interns keys with dense IDs starting from 0', () => {
      ...
      const id1 = table.intern({ world: 'signal', domain: 'number' });
    ...
    it('returns same ID for identical keys (deterministic)', () => {
      const table = new TypeKeyTable();
      const key: TypeKey = { world: 'signal', domain: 'number' };
    ...
    it('differentiates keys by world', () => {
      const table = new TypeKeyTable();
      const id1 = table.intern({ world: 'signal', domain: 'number' });
      const id2 = table.intern({ world: 'field', domain: 'number' });
    ...
    it('differentiates keys by semantics', () => {
      const table = new TypeKeyTable();
      const id1 = table.intern({ world: 'signal', domain: 'number' });
      const id2 = table.intern({ world: 'signal', domain: 'number', semantics: 'unit' });
    ...
    it('supports lookup by ID', () => {
      const table = new TypeKeyTable();
      const key: TypeKey = { world: 'signal', domain: 'number', semantics: 'phase' };
    ...
    it('stable serialization: same key twice produces same ID', () => {
      ...
      // First pass
      const id1a = table.intern({ world: 'signal', domain: 'number' });
      const id1b = table.intern({ world: 'field', domain: 'vec2', semantics: 'position' });

      // Second pass with same keys
      const id2a = table.intern({ world: 'signal', domain: 'number' });
    ...
    it('registers 50 common types without collision', () => {
      ...
      const commonTypes: TypeKey[] = [
        ...
        { world: 'signal', domain: 'number', semantics: 'unit' },
        { world: 'signal', domain: 'number', semantics: 'scalar' },
        { world: 'signal', domain: 'number', semantics: 'count' },
        ...
        { world: 'signal', domain: 'number', semantics: 'phase', unit: '0..1' },
        { world: 'signal', domain: 'number', semantics: 'progress', unit: '0..1' },
        { world: 'signal', domain: 'number', semantics: 'opacity', unit: '0..1' },
        { world: 'signal', domain: 'number', semantics: 'radius' },
        { world: 'signal', domain: 'number', semantics: 'angle', unit: 'radians' },
        { world: 'signal', domain: 'number', semantics: 'angle', unit: 'degrees' },
        { world: 'signal', domain: 'vec2', semantics: 'velocity' },
        { world: 'signal', domain: 'vec2', semantics: 'offset' },
        { world: 'field', domain: 'number', semantics: 'radius' },
        { world: 'field', domain: 'number', semantics: 'opacity' },
        { world: 'field', domain: 'number', semantics: 'scale' },
        { world: 'field', domain: 'number', semantics: 'rotation', unit: 'radians' },
        { world: 'field', domain: 'number', semantics: 'rotation', unit: 'degrees' },
    ...
    it('supports JSON export/import roundtrip', () => {
      ...
      table1.intern({ world: 'signal', domain: 'number' });
