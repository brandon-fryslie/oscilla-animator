...
function createMockPatchStore(
  blocks: Block[] = [{ id: 'time-root', type: 'InfiniteTimeRoot', label: 'TimeRoot', params: {}, position: { x: 0, y: 0 }, form: 'primitive' as const, role: { kind: 'user' as const, meta: {} }}]
): PatchStore {
...
describe('DiagnosticHub', () => {
  ...
  describe('Event Handling', () => {
    ...
    it('should not create missing TimeRoot diagnostic when TimeRoot exists', () => {
      ...
      patchStore = createMockPatchStore([
        { id: 'block-1', type: 'InfiniteTimeRoot', label: 'TimeRoot', params: {}, position: { x: 0, y: 0 }, form: 'primitive' as const, role: { kind: 'user' as const, meta: {} }},
      ]);
      hub = new DiagnosticHub(events, patchStore);
