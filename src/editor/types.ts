...
/**
 * Combine mode including custom modes and first/error.
 *
 * Extends BusCombineMode with additional modes for multi-input port resolution:
 * - 'first': Take first writer (stable ordering)
 * - 'error': Reject multiple writers (compile error)
 * - Custom: Explicitly implemented kernel (e.g. "blendOver", "paletteMix")
 *
 * @see design-docs/now/01-MultiBlock-Input.md §1.1
 */
...
/**
 * Combine policy for input ports.
 *
 * Determines when and how to combine multiple writers:
 * - when: "multi" - Only combine when N >= 2 writers (default)
 * - when: "always" - Always reduce, even with single writer (rare)
 * - mode: "error" - Forbid multiple writers (recovers single-input constraint)
 *
...
export interface Slot {
   ...
   *
   * **Edge ordering**: When order matters ('last', 'first', 'layer'), edges are sorted by
