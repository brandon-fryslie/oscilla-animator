...
describe('BusStore - Event Emission', () => {
  ...
  describe('Bus Binding Events', () => {
    ...
    beforeEach(() => {
      ...
      // Create a test block
      blockId = root.patchStore.addBlock('Constant', root.patchStore.lanes[0].id, { value: 0 });
  ...
  describe('Integration: Bus deletion cascades', () => {
    it('emits BindingRemoved events when bus deleted removes bindings (implicit)', () => {
      ...
      const blockId = root.patchStore.addBlock('Constant', root.patchStore.lanes[0].id, { value: 0 });
